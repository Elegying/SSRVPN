...
安装版最低要求 Windows 10 1507（build 10240）x64，并固定写入
`%LOCALAPPDATA%\\Programs\\SSRVPN`。安装阶段会请求管理员权限，用于验证并结束路径精确
属于当前安装目录的高权限 SSRVPN 应用、启动器和随包 Mihomo；不会按通用名称结束其他
代理/VPN 软件或其他目录中的同名程序。TUN 连接本身同样需要以管理员身份运行应用；这两个
权限边界都与安装包是否签名无关。
