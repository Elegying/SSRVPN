...
macOS 主机和 Android 真机本轮均不可用。对应系统代理、授权、DNS、后台和 OEM 项保持 `blocked`，没有用源码检查、历史证据或自动化结果冒充真机通过。
...
| 平台 | 设备状态 | 本轮证据边界 |
|---|---|---|
| macOS | 无 macOS/Xcode 主机 | 所有 macOS 真机项 `blocked`；只记录 WSL/Linux 可执行的 DNS 事务自动化 |
| Android | `adb devices -l` 无设备 | 所有 Android 真机项 `blocked`；只记录正式 APK 来源、签名、16 KiB 包结构和宿主自动化 |
...
4. 在可用 macOS 主机补 M-02/M-03/M-05 和 30 分钟离线取消；在 Android 真机补 A-01/A-03、原生 16 KiB page-size 和至少两类 OEM 后台限制。
