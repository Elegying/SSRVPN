...
- 不改变“私网安全直连、强制代理、GFW 代理、CN 直连、其他直连”的 IPv4 规则顺序；
- 首页公网 IP 只展示 IPv4。

## 行为约束

1. 生成的 Mihomo 配置必须设置顶层 `ipv6: false` 与 `dns.ipv6: false`，且不得生成 `fake-ip-range6`。
2. 规则列表第一项必须是 `IP-CIDR6,::/0,REJECT,no-resolve`，优先于私网、用户、GFW/CN 和最终直连规则。
...
4. IPv4 局域网排除项保持不变；IPv4 的私网安全、强制代理、GFW/CN 与默认直连顺序保持不变。
