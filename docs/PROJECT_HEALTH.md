...
最近审查：2026-07-14<br>
审查基线：`v3.3.2`，包含 Windows 安装恢复降级与主进程退出后原生代理兜底的完整质量门禁证据。
...
| 用户体验与状态可信度 | 17/20 | 首次导入、连接取消、通知/磁贴同步、桌面代理恢复、受认证的 Android 核心健康检查，以及 Windows 安装恢复留证与死代理自动解除 | 无障碍和多语言仍缺少真机证据；部分错误仍可进一步统一 |
...
| 发布与文档 | 17/20 | 版本/资产/校验和/来源守卫、安装重建的可重试 SHA256 恢复、健康原地升级降级路径、安装与便携包流程及故障指南 | 真实 Windows 安装升级矩阵与付费平台签名尚未完成 |
