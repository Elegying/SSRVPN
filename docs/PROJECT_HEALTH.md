...
- workspace analyze：零问题；版本同步为 `3.4.8+348`；
- shared：353 项通过，覆盖率 `77.98%`（`3580/4591`，门槛 `65%`）；
- Android Flutter：172 项通过，Gradle/JUnit 构建成功，覆盖率 `45.29%`
  （`2027/4476`，门槛 `30%`）；
- macOS Flutter：144 项通过，覆盖率 `59.54%`（`3239/5440`，门槛 `30%`）；
- macOS RunnerTests 通过；生命周期关键文件 `323/492`（`65.65%`，门槛
  `60.00%`），系统代理关键文件 `220/258`（`85.27%`，门槛 `80.00%`）；
- Windows：147 项通过、7 项按平台明确跳过，覆盖率 `49.56%`
  （`3078/6211`，门槛 `30%`）；生命周期关键文件 `24/525`（`4.57%`，门槛 `4.19%`）；
- 发布工具：199 项通过；文档、密钥、核心资产、产品表面、性能和结构守卫全部通过。
