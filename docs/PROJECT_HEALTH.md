...
最近审查：2026-08-22<br>
当前应用版本：`v4.0.15`；公开发布状态与产物以 [GitHub Release](https://github.com/Elegying/SSRVPN/releases/latest) 为准。<br>
`v4.0.15` 候选包含三端连接健康与诊断收尾、Android 生命周期竞态修复、Windows 已验证更新安装包安装后安全清理，以及“关于/使用教程”统一毛玻璃界面。本地门禁、精确 `main` CI、目标平台线上构建与正式 Release 资产必须分别核验；上一正式版的标签、CI、公开资产和实机报告只作为历史证据，不能替代本候选验证。

当前候选工作区于 2026-08-22 完成一次 `make verify` 全门禁；结果见下方“当前候选”证据。它证明本地源码、三端 Flutter/原生测试及覆盖率门槛通过，但尚未绑定最终提交，也不能替代精确 `main` CI、目标平台构建、正式 Release 资产或修复后的人工实机验收。此前 Windows 宿主记录的 49 项共享交互/编辑定向测试、16 项 GitHub-only 更新检查定向测试、Android 236 项、Windows 234 项及 APK/EXE 生成结果属于上一候选的历史证据，不能作为当前候选通过结论。

## 综合结论与评分

**综合评分：93/100（代码健康度评估；不等同于 `v4.0.15` 发布门禁通过、零风险或完整全矩阵实机验收）。**
...
| 测试与 CI | 95 | 当前候选本地全门禁、四套覆盖率与可在 macOS 执行的三端原生测试已通过；精确提交 CI、Windows 宿主测试和发布产物证明待线上回填 |
| 文档与项目治理 | 94 | 文档自动校验、依赖策略、人工 UAT 矩阵、ADR、Issue/PR 与发布边界对齐 |
| 性能与可观测性 | 88 | 有界诊断、带版本与稳定指纹的脱敏崩溃报告、离线关键路径基线具备；真机长期数据仍待采集 |

当前没有发现已知 P0-P1 代码阻断项。残余风险是 v4.0.15 三端正式产物、Windows 安装后自动清理和线上门禁尚待本候选发布流程完成；原生 16 KiB page-size Android 硬件仍待验收。此外，MIUI 12.5 不渲染通知“断开”动作、macOS 持续断网取消专项按用户要求停止，Flutter/Android 上游工具链仍待迁移；这些边界不冒充已执行验收。

## 本轮完成的优化

- 三端把建链硬门禁、核心存活和连接后的外部可达性分层；外网观察失败只发布建议性告警，不断开、不重启、不自动切换。探测与节点切换绑定连接/路由代际，旧结果不能清理或污染新会话，系统代理所有权与数据面告警可独立恢复。
- Android 原生启动失败通过稳定分类传递到 Dart，端口冲突可进入既有的一次换端口重试；连接错误摘要和日志使用稳定、脱敏且可操作的类别，不暴露底层异常。
- Windows 应用内下载且完成 SHA-256 校验的正式安装器使用专属标记和身份校验，在安装事务成功后成对清理安装包与标记；手动、未标记、取消、失败、篡改、超限或身份不匹配文件全部保留。
- “关于”与“使用教程”统一为同一共享透明毛玻璃表面，并完成同视口并排视觉复核、窄窗/大字号/对比度回归；项目链接、开发者、许可证与第三方许可材料同步正式化。
- 发布流水线复用 24 小时内同一精确 `main` 提交已经通过的保护检查，Windows policy 与构建并行并复用工具链缓存；复用只减少重复等待，不跳过精确 SHA、保护分支或正式产物门禁。
...
2026-08-22 在 macOS 26.5.2、Flutter 3.44.1 的 `v4.0.15` 候选工作区执行：
...
- 文档 50 份链接检查、49 份当前状态检查、344 个已跟踪 Dart 文件格式、ShellCheck、核心资产、版本、秘密/TLS 与发布守卫全部通过。
- 发布工具 `374/374`；macOS TUN/DNS 行为 `25/25`；四个 workspace `flutter analyze` 均为 0 issue。
- Shared `643` 项测试通过，覆盖率 `85.07%`（`6290/7394`），门槛 65%。
- Android Flutter `275` 项及 Gradle/JUnit 通过，覆盖率 `67.87%`（`2347/3458`），门槛 30%；完整门禁后又单独重跑 Android 原生套件，补充验证控制 API 的 401/403 均归为凭据不一致而非端口冲突。
- macOS Flutter `281` 项及 RunnerTests 通过，覆盖率 `67.30%`（`3458/5138`）；生命周期 `77.10%`（`616/799`），系统代理 `88.06%`（`391/444`）。
- Windows Flutter `260` 项通过；仅 Windows 主机可执行的 8 项按条件跳过。平台覆盖率 `53.00%`（`3163/5968`），生命周期 `35.66%`（`250/701`），门槛 25%。
- 关键路径 smoke 通过；本机观察值为解析中位数 `6092 us`、合并 `34150 us`、配置生成 `43960 us`，只用于同环境回归，不作为跨机器硬阈值。

该证据尚未绑定最终提交；提交后必须由精确 SHA 的受保护 CI 和目标平台在线构建再次验证。
...
### 2026-08-20 v4.0.13 正式发布历史快照

- `v4.0.13` 标签精确指向 `aa157395f87959696ad21e6a5d894f54c400cb82`；标签前 `main` CI `32304327190` 与正式 Release `32305723229` 全部通过，当时公开 Release 为非草稿、非预发布且是 `latest`。这只是历史快照；当前公开版本仍以文首 GitHub Release 链接为准。
...
- 当次验收时 OSS `latest.json` 指向 `4.0.13`；不可变版本路径与网站固定下载别名重新下载后均与 GitHub Release 摘要一致。官网 macOS 页面继续明确仅支持 Apple M 系列芯片、不支持 Intel Mac。

## 证据边界与残余风险

- Android 17 已完成 v4.0.13 修复项增量实机复验；Windows 11 报告基线完成覆盖安装、数据保留、连接和断开，但报告后修复的三项行为尚未使用最终正式安装器人工复验。未重复的长时、耗电和完整压力矩阵继续保持未执行。
- 历史正式版已分别建立候选、版本准备、合并后 `main`、标签前精确 `main`、发布专用 CI 和 Release 提交证据；`v4.0.15` 发布结论不得复用这些旧版本绿色 run。
- GitHub 不提供 `main` 与 tag 跨引用的原子推送；从最终 `main` 复核到 Release `Validate release source` 通过前必须暂停合并 `main`。若该窗口内 `main` 前移，不可变 tag 只能废弃并升新版本创建新 tag，不能删除、移动或重跑原 tag。
- macOS 本机不能执行 Windows C++、PowerShell 5.1、DPAPI、注册表和 Inno Setup；历史正式版源码和安装器曾由 Windows CI 覆盖，但当前 `v4.0.15` 仍须等待自己的 Windows CI，且自动化结果不能表述为修复后的 Windows 人工实机验收。
