...
| 维度 | 得分 | 当前证据 | 主要缺口 |
| --- | ---: | --- | --- |
| 正确性与恢复 | 19/20 | 连接代际、取消、核心退出、订阅事务、系统代理所有权和更新回滚均有测试或守卫；Windows 主进程退出后还有启动器原生恢复与自有死端点禁用兜底 | 仍需扩大真实弱网和休眠/唤醒矩阵 |
| 安全与信任边界 | 18/20 | 外部输入有界、日志脱敏、资产摘要、Android/Windows 系统凭据保护、桌面 secret 验证迁移与崩溃一致性、Windows 每用户写入边界、macOS 最小文件权限、TUN 临时授权边界 | macOS/Windows 未受信任签名；桌面运行时配置仍有同用户可读残余风险 |
| 用户体验与状态可信度 | 17/20 | 首次导入、连接取消、通知/磁贴同步、桌面代理恢复、受认证的 Android 核心健康检查，以及 Windows 安装全新覆盖与死代理自动解除 | Windows 安装升级不保留用户配置；无障碍和多语言仍缺少真机证据 |
...
| 发布与文档 | 17/20 | 版本/资产/校验和/来源守卫、Windows 全新覆盖安装守卫、安装与便携包流程及数据清除警告 | 真实 Windows 安装升级矩阵与付费平台签名尚未完成 |
