...
Windows 安装器不再把其他本地代理工具设置的系统代理误判为 SSRVPN 自有。端点归属改为要求本安装自身的恢复记录佐证，不再仅凭「回环端点 + 常见旁路列表」的形状推断；从未运行过 SSRVPN 的全新机器不再在安装准备阶段失败关闭并报 `PROXY_UNSAFE`。有恢复记录时的失败关闭语义完整保留，第三方代理／VPN 软件不受影响。
硬性规则清单的编号重号、漏编与错位已修正为 1..56 连续，并新增门禁守卫要求该清单严格连续递增。
