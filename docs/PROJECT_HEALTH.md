...
最近审查：2026-07-14<br>
审查基线：`v3.3.3`，包含 Windows 无提权全新覆盖安装与主进程退出后原生代理兜底的完整质量门禁证据。
...
| 可维护性与自动验证 | 19/20 | workspace analyze、四套 Flutter 测试、Android 原生测试、发布工具测试、边界脚本和覆盖率门槛均纳入 `make verify`；共享包启用推荐 lint，窗口状态和启动日志公共逻辑已用行为测试提取 | 大型桌面 UI/服务文件仍需按行为切片，不能为重构而重构 |
