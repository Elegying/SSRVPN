...
最近审查：2026-08-22<br>
当前应用版本：`v4.0.16`；公开发布状态与产物以 [GitHub Release](https://github.com/Elegying/SSRVPN/releases/latest) 为准。<br>
`v4.0.16` 候选把三端节点延迟测试统一恢复为 TCP 端口握手口径，并保持“私家车”节点既定显示规则不变。本地门禁、精确 `main` CI、目标平台线上构建与正式 Release 资产必须分别核验；上一正式版的标签、CI、公开资产和实机报告只作为历史证据，不能替代本候选验证。
...
**综合评分：93/100（代码健康度评估；不等同于 `v4.0.16` 发布门禁通过、零风险或完整全矩阵实机验收）。**
...
当前没有发现已知 P0-P1 代码阻断项。残余风险是 v4.0.16 三端正式产物、Windows 安装后自动清理和线上门禁尚待本候选发布流程完成；原生 16 KiB page-size Android 硬件仍待验收。此外，MIUI 12.5 不渲染通知“断开”动作、macOS 持续断网取消专项按用户要求停止，Flutter/Android 上游工具链仍待迁移；这些边界不冒充已执行验收。
...
2026-08-22 在 macOS 26.5.2、Flutter 3.44.1 的 `v4.0.16` 候选工作区执行：
...
- 历史正式版已分别建立候选、版本准备、合并后 `main`、标签前精确 `main`、发布专用 CI 和 Release 提交证据；`v4.0.16` 发布结论不得复用这些旧版本绿色 run。
...
- macOS 本机不能执行 Windows C++、PowerShell 5.1、DPAPI、注册表和 Inno Setup；历史正式版源码和安装器曾由 Windows CI 覆盖，但当前 `v4.0.16` 仍须等待自己的 Windows CI，且自动化结果不能表述为修复后的 Windows 人工实机验收。
