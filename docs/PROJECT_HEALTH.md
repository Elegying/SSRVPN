...
本轮增加独立签名规则通道、Android 应用名单优先分流、规则失败回退及订阅策略隔离，
完善输入与连接恢复，减少玻璃纹理和图层重复工作。最新审查见
[最终发版前审查](uat/SSRVPN_最终发版前审查_20260915.md)。
当前仍在准备正式候选，最终提交 CI、规则快照同步与发布产物核验完成前不视为已发布。
