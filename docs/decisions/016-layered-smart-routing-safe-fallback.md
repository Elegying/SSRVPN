...
- 启动时只修复缺失、链接、超限或语法无效的本地 provider。有效缓存原样保留，内置规则准备
  失败只记稳定告警，不阻断核心启动；只有本地清单与六份活动文件完全匹配时才认定已安装版本。
...
- 清单通过后按新旧文件摘要只请求 Mihomo 刷新内容实际变化的 SSRVPN 自有 provider；本地活动
  版本未知时才保守刷新精确六份，GFW/CN 固定快照不参与重复刷新。provider 文件不嵌入会让
  未变内容摘要漂移的通道总版本号。六份活动文件全部再次通过 SHA-256、条目数、语法和大小
  验证后，才写入本地活动清单。
  HTTP、解析、刷新、断线或落盘失败均保留原版本记录和有效文件，不影响已经建立的连接；下次
  连接继续重试。
...
- 共享测试解析完整配置并固定检查用户冲突、各层顺序、DNS policy、`MATCH,PROXY`、规则包
  安装、版本相同零下载、只允许更高版本、只刷新变化 provider、活动清单最后提交、有效缓存
  保留和损坏缓存恢复。
