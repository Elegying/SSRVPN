...
- 剩余的失败关闭出口（恢复日志残留 pending 标志、`APP_INSTANCE_ACTIVE`、进程身份无法
  ...
  单独处置。
- 本决策不改变硬规则 #11 中「应用退出与异常恢复仍只结束路径、PID、会话、创建时间和
  所有权能够证明属于当前 SSRVPN 安装的进程」的边界。

> 2026-09-22 追记：本 ADR 决策 4 与上文「退出其他目录或便携版实例」的可执行建议，已在
> 安装/卸载停进程范围内被 [ADR-021](021-installer-name-based-process-stop.md) 按产品决策
> 取代（安装/卸载按精确映像名结束随包进程，含其他目录与便携版副本）；系统代理归属判定
> 语义不变。故上文「安装、卸载与恢复仍只结束路径…」的旧表述仅对「应用退出与异常恢复」
> 继续成立，安装与卸载的停进程范围以 ADR-021 与硬规则 #11 现文为准。
