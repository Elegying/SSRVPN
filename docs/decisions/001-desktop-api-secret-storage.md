...
- Windows 便携模式保留可移动数据的产品特性，但移动或共享整个便携目录会同时移动订阅、
  缓存、DPAPI 密文和运行时配置，用户必须自行保护该目录。程序目录回退到 LocalAppData、
  安装器修复和覆盖升级都必须逐字节验证并保留 `.api-secret.dpapi`；目标目录随后的实际
  DPAPI 回读仍是迁移成功的最终门槛。
- macOS 公开包当前使用 ad-hoc 签名，没有稳定签名身份、provisioning profile 或
  ...
  提示或因用户拒绝导致启动失败。配置稳定 Developer ID 后，应以新 ADR 设计从
  `.api-secret` 到 Keychain 的验证后迁移与回滚。
