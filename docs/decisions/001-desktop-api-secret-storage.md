...
已接受；Windows 安装器的数据清理与保留条款由 ADR-002 取代
...
4. Mihomo 运行期间的 `config.yaml` 仍必须包含控制接口 secret：
   ...
   - Windows 运行时文件继承其所在数据目录的 ACL。安装版数据目录应保持为当前用户私有，
     不应放入公共共享目录。
...
- Windows 便携分发已由 ADR-003 取代；遗留数据目录仍可能同时包含订阅、缓存、DPAPI 密文
  和运行时配置，用户必须自行保护。`v3.3.3`–`v3.4.1` 的安装版曾按全新覆盖策略清除旧安装
  数据；自 `v3.4.2` 起，该安装器条款由保留策略取代：覆盖升级、
  卸载以及保留数据后的重装保留 `.api-secret.dpapi`、订阅、设置和窗口状态，只清理程序文件、
  旧恢复状态与已知 WebView 缓存。这个变更不影响应用运行期间使用当前用户 DPAPI 保护长期
  secret 的决策，桌面或下载目录中的便携副本也不会被安装器搜索或修改。
