...
| 轮次 | 方向 | 证据 | 结论/修复 | 状态 |
| --- | --- | --- | --- | --- |
| N1 | 安装流程与每用户权限 | L1 `test_windows_installer_config.py`；L2 `test_windows_installer_runtime.ps1`；L3 安装包 smoke 已进入 CI | 修复直接卸载不恢复代理/不停止主程序；停止器只处理固定安装路径，安装仍固定 `%LOCALAPPDATA%` 且无提权 | 本地关闭，待 L3 |
...
| N5 | 编码、本地化与 PS5.1 | L1 ASCII/显式编码/BOM/`/utf-8` 守卫；L2 UTF-8 与 PS5.1 测试 | 修复 PowerShell 非终止错误假成功、Mihomo 非法 UTF-8 导致流中断；安装器完整中文化 | 本地关闭，待 L3 |
...
| N9 | CI、打包、发布与供应链 | L1 固定 action/Gitleaks/发布顺序；L2 发布工具 69 项 | Windows CI 新增真实安装/卸载；GitHub Draft、OSS 不可变目录、备份推广与失败补偿保持完整 | 本地关闭，待 L3 |
...
| --- | --- | --- | --- | --- |
| A1 | PS5.1 与敌对代码页 | L1 禁止非 ASCII 脚本和不兼容 `Split-Path`；L2 PS5.1 round-trip | 所有嵌入命令先设 `ErrorActionPreference=Stop` 和 UTF-8；JSON `-Raw` 必须显式编码 | 本地关闭，待 L3 |
| A2 | 锁文件、杀软延迟、半安装、Unicode 路径 | L2 同名多副本故障注入；L3 package smoke | 修复误杀便携同名程序；锁未释放时在 `[InstallDelete]` 前失败，不再用 `restartreplace` | 本地关闭，待 L3 |
...
| A10 | 干净机、运行库缺失、未签名提示、日志与回滚 | L1 VC runtime 打包、SHA256、免费分发守卫；L3 clean runner | 不购买证书边界已统一；CI 验证干净 runner 安装/卸载，用户文档保留 SmartScreen 说明与 `v3.3.5` 回滚 | 本地关闭，待 L3/L4 |
...
| 兼容性 | PowerShell 5.1 参数集、ASCII 源、UTF-8 输入输出、Inno Unicode 和 MSVC `/utf-8` 均有守卫。Inno 官方说明 Restart Manager 只检查 `[Files]`/`[InstallDelete]` 中要更新的资源，因此不会仅因同名关闭其他目录的便携副本。 |
...
Windows 原生编译、PowerShell 5.1、安装/卸载和公开产物结论待线上 L3 完成后补入。
L4 Windows 11 交互安装、连接、断开、重启与隔夜验证仍是明确缺口，不能由 CI 替代。
