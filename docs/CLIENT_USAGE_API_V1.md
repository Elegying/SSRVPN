...
- 客户端「已用流量」展示 `用量/额度 百分比`，例如 `125GB/250GB 50%`。用量和额度分别读取 `usedBytes`、`trafficLimitBytes`，按现有 1024 进位独立选择 B 至 EB 单位；卡片内用量/额度与百分比分行，下方固定提醒「每月1日重置」。该提醒为产品文案，不参与客户端清零；用量始终由面板响应确定。比例由原始字节数计算、不截断超额，保留至一位小数并去除尾随 `.0`；非零不足 0.1% 显示 `<0.1%`，达到 10000% 使用三位有效数字的科学计数法。零额度显示 `—%`，不假定为无限额度；无障碍语义保留两个原始字节数。
...
- 后台/离开首页暂停查询，保留同账号尚未过期的完整成功快照；恢复时检查原有效期并继续有界刷新。有效期仍由单调时间管理，暂停期间的墙钟耗时只用于保守扣除休眠时间，不得延长缓存有效期；墙钟倒退时丢弃快照。失败、过期或身份变化仍立即隐藏两块。
