...
- 后台/离开首页暂停查询，保留同账号尚未过期的完整成功快照；恢复时检查原有效期并继续有界刷新。有效期仍由单调时间管理，暂停期间的墙钟耗时只用于保守扣除休眠时间，不得延长缓存有效期；墙钟倒退时丢弃快照。失败、过期或身份变化仍立即清除旧数字；v5.0.12 起适用账号节点保留两块占位，普通节点隐藏两块。
...
稳定代码包括 `INVALID_CREDENTIALS`、`STATS_UNAVAILABLE`、`STATS_INCOMPLETE`、`STATS_STALE`。HTTP 非 200、解析异常、不完整、超时、断网、限流、认证失败均立即清除两个账号卡片的旧数字，保留占位并显示暂未更新；悬停或长按可查看简明原因，诊断日志只记录脱敏的失败类型变化和恢复，不记录 URL、凭据或异常原文。401 也采用有界退避，凭据/订阅更新立即重新校验。429 遵守 `Retry-After`（秒；也接受带响应 Date 的 HTTP 日期形式）。HTTP 200 的错误对象不会被当成功。
