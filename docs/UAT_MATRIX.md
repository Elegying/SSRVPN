...
| C-11 | 订阅刷新结果生命周期 | 成功或失败提示立即显示，只保留最新结果，并在显示 10 秒后自动消失 | 成功/失败在第 0、9、10 秒的截图或可控时钟测试 |
| C-12 | 智能/全局代理模式说明 | 三端选中模式、实际配置和副标题一致；智能显示国内直连说明，全局显示所有流量走代理说明 | 三端模式截图、配置或运行日志 |
...
| A-07 | 添加订阅输入框的键盘与编辑菜单 | 聚焦后使用系统默认键盘；中文环境长按显示“粘贴”而不是 `Paste`，粘贴可正常完成且订阅内容不进入日志或诊断 |
| A-08 | 连接中切换智能/全局模式与节点 | 连续至少 5 轮 `智能 → 全局 → 智能` 及模式/节点交替切换后仍保持最后一次连接意图；应用状态、VPN Service 和真实 HTTPS 数据通道一致；重载失败时恢复原连接或明确提示已断开，不得静默失去 VPN |
...
| ID | 场景 | 通过标准 |
| --- | --- | --- |
| W-01 | 普通用户首次安装、启动和卸载 | 默认每用户路径工作，卸载不删除用户数据 |
