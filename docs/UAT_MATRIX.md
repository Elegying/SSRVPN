...
本地只读证据采集与时序汇总可使用 `scripts/collect_uat_evidence.py`；命令、退出码和本轮真实边界见
[2026-08-20 综合审查建议闭环记录](UAT_REMEDIATION_20260820.md)。带 `--require-16k` 或
`--require-battery-ready` 的 Android 预检在条件不满足时返回退出码 2，禁止改成软通过。
...
- 同一设备上的正式 v4.0.13 APK 另完成 20/20 轮连接、真实 HTTP 204 和断开；无崩溃、卡死或 7890/7891 残留，详细时序与外部触控排除证据见综合建议闭环记录。
