...
## 2026-09-24：首页外部探测提示边界

自动化覆盖三端真实首页组件及桌面 App 外壳：启动前已有缓存告警、连接后探测失败、
本地代理告警同时存在/独立恢复、核心或 VPN 停止。外部探测失败时首页保持真实连接状态；
诊断和日志显示“外部探测未通过，实际访问情况尚未确认”。Android 系统验证失败与物理
网络确实不可用分开。横向矮窗口叠加故障通知仍须完整显示连接按钮。
本轮按维护者要求不再执行三端人工实机验证；这些条目的人工结果保持未执行，自动化
结果和 Windows 隔离安装验收分别在 PROJECT_HEALTH.md 与 CI 留存。

## 2026-09-23：启动恢复上次延迟

自动化已覆盖三端实际主页：手动批量测速后销毁页面、重新从磁盘创建订阅服务，
主页与节点列表显示相同结果及测试时间，包含订阅内容未变时刷新后再次测速，
且不自动发起测速；同名节点地址变化后
仅该节点恢复未知。共享回归覆盖单点结果、失败类别、取消、迟到回调、刷新提交并发、
缓存损坏与写入失败。下表专指安装候选包后的实机验收，不能用自动化结果代替。

| 平台 | 实机步骤 | 预期 | 实机结果 |
| --- | --- | --- | --- |
| Android | 单点及批量测速后退出进程再启动；修改某节点端口后再次启动 | 复用原结果；未测试及端口已变节点未知；不自动测速；手动重测覆盖旧值 | 未执行 |
| macOS | 同上，检查主页与节点列表 | 同上 | 未执行 |
| Windows | 同上，检查正常退出后重新启动 | 同上 | 未执行 |

## 2026-09-16：5.0.6 本地候选实机验证

本轮状态见 [5.0.6 候选实机验证报告](uat/SSRVPN_v5.0.6_候选实机验证报告_20260916.md)。
另见 [5.0.6 增量实机验证报告](uat/SSRVPN_v5.0.6_增量实机验证报告_20260916.md)，覆盖连接过程提示、设置/显示交互和新增网络提醒核查。
该报告明确列出已验证、失败后修复及未完成项。以下各版本记录为历史证据，不代表当前候选已通过完整验收。

## 历史正式版本 v4.0.36：发布通过，完整新版实机未执行
...
三端报告交叉核对、Windows F-01 修复和发布审查见
[三端复核报告](uat/SSRVPN_三端报告复核与发布审查_20260907.md)。
[Mac 与 USB Android 验收](uat/SSRVPN_审查修复与实机验收_20260906.md) 和下方 Windows v4.0.34
记录分别绑定当时的候选/正式包，不直接改写为新版本全部实机通过。未执行和阻塞项目继续保留。
...
## 2026-09-06 至 09-07 v4.0.34 Windows 实机验收

- 使用正式 Windows `4.0.34+4034` 安装版和维护者提供的 Hysteria2 节点完成本机验证，
  正式包摘要/provenance 与安装文件清单一致。完整结果和逐轮证据见
  [v4.0.34 Windows 实机验收报告](uat/SSRVPN_Windows_v4.0.34_实机验收报告_20260906.md)。
- 系统代理连续 20/20 轮连接/断开、实际 HTTPS 204、代理恢复和资源清理通过；
  TUN/系统代理模式、GUI 异常退出恢复、同版本覆盖、卸载保留数据和正式重装取得实机证据。
- **C-10 失败：诊断报告遗漏双反斜杠 Windows 路径中的本机用户名脱敏**。
  实际剪贴板读回与共享实现的最小复现一致，按 P2 交接 Mac 端修复，不标记完整 Windows 验收通过。
- 未按其他版本结果填补本轮缺口；按维护者要求未执行断网验证，UAC 弹窗、重启恢复及其他
  未执行/阻塞子场景在报告中逐项保留。自动化与部分子场景通过不替代完整实机矩阵。
...
- Android“智能”模式国内应用精确旁路增至 182 个，新增汽车之家、懂车帝、飞猪和易车；
  常用浏览器、国外应用、VPN 和特权工具继续进入 TUN，“全局”模式同样保留国内应用旁路。
...
| V24-02 | Android 智能/全局 | 浏览器、Telegram、国内与国外应用 | 浏览器和国外应用继续进入 TUN；全局模式同样保留国内应用旁路 | 自动化通过 / 真机未执行 |
...
- Android 仅在“智能”模式把 178 个经审查的国内应用排除在 TUN 外，包含抖音及轻量版；
  ...
  不确定包名均有“不旁路”负例。“全局”模式同样保留国内应用旁路；ADB 调试包保留既有旁路。
...
| V23-03 | Android 全局 | 国内与国外应用 | 国内应用旁路保留（与智能模式一致），名单内应用不进入 TUN；其余应用与 ADB 调试包进入 TUN | 自动化通过 / 真机未执行 |
