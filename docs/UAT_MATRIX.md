...
## 2026-09-03 v4.0.26 规整日志与规则文件持久化发布候选
...
| V26-03 | 三端连接后规则升级 | v4.0.24 旧缓存、同版、失败和重连 | 新内容长期落盘；清单最后提交；失败不断连且下次可重试 | 自动化通过 / 真机未执行 |

上述状态只代表候选自动化证据；受保护合并、正式线上三端构建、公开资产和实际设备复验将在
完成后补录，不预先标记为通过。
