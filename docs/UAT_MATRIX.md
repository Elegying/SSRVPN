...
## 2026-09-03 v4.0.25 规整日志与规则文件持久化发布候选
...
| --- | --- | --- | --- | --- |
| V25-01 | Android、macOS、Windows UI | 订阅页、节点选择页、窄窗和大字号 | 订阅页显示“运行日志”文字且可打开诊断；节点页无重复入口 | 自动化通过 / 人工未执行 |
| V25-02 | 三端诊断 | 正常、有提醒、错误及技术明细 | 默认内容简明规整且不泄露内部标识；原始脱敏信息仍可按需展开 | 自动化通过 / 人工未执行 |
| V25-03 | 三端连接后规则升级 | v4.0.24 旧缓存、同版、失败和重连 | 新内容长期落盘；清单最后提交；失败不断连且下次可重试 | 自动化通过 / 真机未执行 |
