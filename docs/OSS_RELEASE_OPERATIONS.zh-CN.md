...
在 GitHub Actions 手动运行 `Maintenance`，选择 `oss-smoke`。它只更新
...
2. 在 GitHub 仓库 Settings → Secrets and variables → Actions 中替换两项
   `ALIYUN_OSS_*` Secret。
3. 手动运行 `Maintenance` 的 `oss-smoke` 任务，确认新密钥可用。
4. 再禁用并删除旧 AccessKey。
...
- 已发布版本确认有严重问题：先把 GitHub Release 标记为 prerelease，再在
  GitHub Actions 手动运行 `Maintenance` 的 `oss-rollback` 任务，输入上一个稳定 tag。
