...
**Windows 独有的过时依赖**：
Windows 本体不直接使用 `package:uuid`，已删除直接依赖。实际 `uuid` 版本由 `ssrvpn_shared` 和 `system_tray` 解析，避免与 `system_tray 2.0.3` 的 `uuid ^3.0.6` 约束冲突。
...
### 4.5 macOS Swift Package Manager

macOS 项目已移除 CocoaPods 集成文件，改用 Flutter Swift Package Manager 集成，减少 CI 和本地构建依赖。
...
| 项目 | 状态 |
|------|------|
| Monorepo tag | ✅ 已有 v2.0.6 |
| GitHub Release | ✅ v2.0.6 资产齐全 |
| Android 签名自动化 | ✅ 支持免费自签名 keystore secrets |
| 公证自动化 (macOS) | ⚪ 当前免费发布策略不启用 |
| 代码签名 (Windows) | ⚪ 当前免费发布策略不启用 |
| CI 构建 | ✅ 正常 |
| Release 工作流 | ✅ 已就绪，含核心资产校验和产物校验 |
...
| **安全性** | 7/10 | ↑ | Android secret 存储较好；桌面端本地明文是可清理性取舍 |
| **发布就绪** | 8/10 | ↑ | 已有 tag/release，Android 可免费签名；macOS/Windows 会有系统提示 |
| **整体** | **7.6/10** | ↑ | 基础扎实，主要瓶颈转向代码去重和平台烟测 |
...
2. **真实设备/干净机器烟测 v2.0.6 产物** — Android 覆盖安装、macOS Gatekeeper 路径、Windows 绿色包启动

### P1 — 本周

3. **各平台 ClashService 继承 ClashServiceBase** — 消除 ~2,000 行重复
4. **统一 ProxyMode 枚举** — 消除 4 处重复定义
5. **继续扩展共享 Update/Config 测试** — 防止三端行为漂移

### P2 — 本月

6. **AppSettings 基类提取** — 消除 ~500 行重复
7. **HomeScreen 组件拆分** — 提升可维护性
8. **发布说明加入免费发布安全提示** — macOS 右键打开、Windows SmartScreen、Android 自签名升级约束
9. **完善 release notes 生成** — 从 CHANGELOG 提取用户可读内容

### P3 — 后续

10. **网站添加版本号显示**
11. **网站 SEO** — 确认 noindex 是否有意为之
12. **集成测试** — Clash 配置生成 + API 组切换
13. **按需评估付费签名** — 仅在分发规模需要降低系统警告时再做
