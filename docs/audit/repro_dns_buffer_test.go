...
type auditDNSService struct{}

func (auditDNSService) ServeMsg(_ context.Context, q *D.Msg) (*D.Msg, error) {
	r := new(D.Msg)
	r.SetReply(q)
	for i := 0; i < 120; i++ {
...
func TestAuditDNSPackedBytesRemainInCallerBuffer(t *testing.T) {
	previous := DefaultService
	DefaultService = auditDNSService{}
