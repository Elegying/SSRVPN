...
订阅 URL、节点信息、缓存和其他非密钥设置仍保存在应用数据目录；Windows 安装版覆盖升级保留固定安装数据，但安装器不搜索或合并其他便携/回退来源，Windows 便携版的数据目录位于程序旁。不要共享整个数据目录，并确保便携目录只对可信的当前用户开放。安装版边界见 [ADR 002](docs/decisions/002-windows-installed-data-preservation.md)。
