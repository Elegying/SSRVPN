...
涉及凭据泄露、代理绕过、权限提升、更新链路、崩溃转储或签名材料的问题，不要创建公开 Issue。优先使用 [GitHub 私有漏洞报告](https://github.com/Elegying/SSRVPN/security/advisories/new) 联系维护者，并提供受影响平台和版本、最小复现步骤、预期与实际行为，以及已经脱敏的日志或截图。
...
不能宣称达到 Keychain 的保护强度。由于项目不购买 Developer ID，本存储边界保持不变；
若未来明确改变分发决策，必须先用新 ADR 设计迁移与回滚。依据见
...
订阅 URL、节点信息、缓存和其他非密钥设置仍保存在应用数据目录；Windows 安装版覆盖升级保留固定安装数据，但安装器不搜索或合并旧独立副本与回退来源。不要共享整个数据目录，并确保目录只对可信的当前用户开放。安装版边界见 [ADR 002](docs/decisions/002-windows-installed-data-preservation.md)。

Mihomo 运行时 `config.yaml` 必须包含 API secret 才能提供受认证控制接口，因此核心运行期间仍存在一份短期明文：macOS 以独占方式创建空临时文件，先将权限设为 `0600`，再写入含 secret 的内容并原子替换正式配置；Windows 继承应用数据目录 ACL。同一系统用户下的其他进程仍可能读取该文件，这是当前残余风险，不应描述为“完全消除明文”。决策与回滚边界见 [ADR 001](docs/decisions/001-desktop-api-secret-storage.md)。
