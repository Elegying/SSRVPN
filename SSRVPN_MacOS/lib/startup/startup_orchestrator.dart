...
class StartupOrchestrator {
  ...
  Future<void> start() async {
    ...
    await runStep('window_manager', initWindowManager, skip: flags.safeMode);
    await runStep('screen_retriever', initScreenRetriever,
        skip: flags.safeMode);
    await runStep('system_tray', initSystemTray, skip: flags.disableTray);
    await runStep(
      'mihomo_core',
      initCoreService,
  ...
  Future<void> _quitFromTray() async {
    ...
    if (!isMacosAppShutdownSafeToExit(failures)) {
      try {
        ...
        StartupLogger.error(
          'Show window after failed tray quit cleanup',
          error,
          stack,
        );
      }
