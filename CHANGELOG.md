...
- macOS TUN 模式由本次授权的 root runner 将当前默认网络服务 DNS 临时接管到 Mihomo 的 `127.0.0.1:53`，断开、核心退出、授权取消或应用退出时恢复原来的自动/手动 DNS；恢复前会复核所有权并有界重试，不覆盖用户中途修改的设置。
...
- macOS TUN 启动时会在存在遗留恢复标记的情况下先请求管理员授权恢复 DNS；运行期健康检查同时验证 Mihomo API、提权 runner 状态与节流的数据通道探测。runner 或 DNS 恢复失败时客户端立即回到未连接状态、保留恢复证据并阻止再次启动，不再把清理失败误报为成功断开。
- 更新文件发布增加同目录中断恢复：下一次下载会先恢复缺失的已校验目标；Windows 同时清理一天前遗留的受严格命名约束的 `.part`，并在目标缺失时恢复对应 `.previous`，不触碰近期、无关、目录或符号链接文件。
- GitHub 发布流程会识别并恢复同一标签下中断留下的不完整 Draft Release，对瞬时 API 故障采用有界重试，避免重新发布时因隐藏草稿或半成品状态持续失败。
...
- 新增三端共享页面、两页导航、离线节点预选、连接状态竞态、订阅错误脱敏与紧凑大字体弹窗、Android 诊断状态、Windows 真实桌面解析/版本化文件名/取消下载/中断文件恢复，以及 TUN DNS 配置、启动遗留恢复、停止恢复失败、runner 存活、运行期数据面、DNS 分类错误、连接中切换自动重连和数据面失败不提交运行状态的回归测试；特权 runner 同时通过语法、ShellCheck、固定摘要、行为级故障注入与静态安全门禁，发布工具测试覆盖 annotated tag 拒绝与提交剥离。
