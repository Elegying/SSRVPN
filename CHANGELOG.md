...
- TUN DNS 接管只接受当前默认接口对应的有效网络服务和规范 IP 地址；原 DNS 快照保存在 root-only 运行目录，所有 `networksetup` 参数均以独立参数传递，runner 资源摘要继续由应用固定并在提权后复核。

### 测试

- 新增 TUN DNS 配置、直连数据面、DNS 分类错误、连接中切换自动重连和数据面失败不提交运行状态的回归测试；特权 runner 同时通过语法、ShellCheck、固定摘要与静态安全门禁。
