...
2026-09-06 的扩展仅增加首页统计，不改变分流规则，也不改变 Mihomo
...
仍须匹配来源清单中的二进制 SHA-256。Go 下载归档的长度和摘要另由
...
在 Windows 主机上安装 Android/macOS 交叉编译工具链。应用版本仍为 v4.0.34；
源码同步不创建标签、不触发 Prepare Release、不更新 GitHub/OSS 安装包。
