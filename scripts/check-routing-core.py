...
def main():
    with ExitStack() as stack:
        ...
        while mixed == api:
            api = traffic.free_port()
        shutil.copytree(ROOT / 'packages/ssrvpn_shared/assets/rules/latest', folder / 'providers/bundles/2.0.0')
        environment = dict(os.environ, SSRVPN_ROUTING_FIXTURE=str(folder), SSRVPN_ROUTING_PORTS=json.dumps({'proxy': proxy, 'mixed': mixed, 'api': api}))
        subprocess.run(['flutter', 'test', '--no-pub', 'packages/ssrvpn_shared/test/signed_rule_snapshot_test.dart'], cwd=ROOT, env=environment, check=True, stdout=subprocess.DEVNULL)
            ...
            with (folder / f'{mode}.log').open('w+') as log:
                ...
                try:
                    ...
                    for _ in range(100):
                        ...
                        raise AssertionError(('providers not ready', providers))
                    def sample():
