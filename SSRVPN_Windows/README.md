...
便携副本，也不需要管理员权限。覆盖升级只替换已知程序文件，保留安装目录
`bin\ssrvpn`、`%LOCALAPPDATA%\SSRVPN\ssrvpn`、窗口状态以及其中的订阅、设置和 DPAPI 密钥；
旧安装恢复状态和 WebView 缓存会被清理。向导使用仓库内置简体中文语言文件，并在复制前明确提示
替换和保留范围。安装器会先恢复自有系统代理并关闭旧进程；若文件仍被占用或代理无法安全恢复，
会在替换程序文件前停止并提示退出程序或重启 Windows，不依赖需要管理员权限的重启替换。

CI 会在 Windows Server runner 上用系统自带 Windows PowerShell 5.1 解析全部脚本，
再以普通用户、默认 `%LOCALAPPDATA%\Programs\SSRVPN` 路径实际静默安装、覆盖升级和卸载刚生成的
安装包。该检查验证安装器退出码、关键文件、升级数据保留、WebView 缓存清理、启动后安全清理和卸载残留；它不能替代
Windows 10/11 真机上的交互向导、连接、断开、重启与系统代理验收。
