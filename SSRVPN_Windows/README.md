...
便携副本，也不需要管理员权限。自 3.3.3 起，每次安装都会清空固定安装目录、
LocalAppData 回退配置、窗口状态和旧安装恢复状态，不再备份或恢复旧订阅与设置；
安装完成后需要重新导入订阅。向导使用仓库内置简体中文语言文件，并在复制前明确提示
删除范围。安装器会先恢复自有系统代理并关闭旧进程；若文件仍被占用，会在删除旧数据前
停止并提示退出程序或重启 Windows，不依赖需要管理员权限的重启替换。

CI 会在 Windows Server runner 上用系统自带 Windows PowerShell 5.1 解析全部脚本，
再以普通用户、默认 `%LOCALAPPDATA%\Programs\SSRVPN` 路径实际静默安装和卸载刚生成的
安装包。该检查验证安装器退出码、关键文件、启动后安全清理和卸载残留；它不能替代
