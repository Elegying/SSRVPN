...
- TUN 模式需要管理员权限，用于游戏、桌面客户端及其他不读取系统代理的程序。管理员账户
  以普通权限运行时，点击连接会直接显示 Windows UAC 授权窗口；确认后 SSRVPN 会安全重启
  并自动继续本次连接。授权通过后界面会明确提示切换状态，当前窗口会短暂关闭；请等待
  管理员窗口自动重新打开，不要重复启动软件。取消授权时旧实例保持打开。标准用户或改用
  其他管理员账户授权无法读取当前用户的 DPAPI 数据，因此不会自动接管；请改用管理员账户
  登录 Windows 后运行。
