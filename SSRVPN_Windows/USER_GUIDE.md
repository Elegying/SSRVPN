...
> **安装版升级数据：** 覆盖运行 `SSRVPN_Setup.exe` 会替换固定安装目录中的
> 已知程序文件，但保留 `bin\ssrvpn`、`%LOCALAPPDATA%\SSRVPN\ssrvpn`、窗口状态以及
> 其中的订阅、设置和 DPAPI 密钥。旧安装恢复状态和 WebView 缓存会被清理。
> 桌面、下载目录或其他位置的便携副本不会被搜索或修改。

安装向导使用简体中文，并在复制前展示上述替换和保留范围。若旧 SSRVPN 进程无法安全关闭，
安装会在替换程序文件前停止；请退出托盘实例后重试，仍失败时重启 Windows 再安装。
