...
Windows 只发布 `SSRVPN_Setup.exe`。默认安装目录为当前用户的
`%LOCALAPPDATA%\Programs\SSRVPN`，可在目录选择页调整，并创建桌面和开始菜单入口。为安全确认并停止已提权的
...
安装器不会搜索或合并桌面、下载目录等位置的旧数据。安装和卸载期间，会关闭映像名精确为
`ssrvpn_windows_app.exe`、`ssrvpn_windows.exe`、`mihomo.exe` 的运行实例，包括其他目录
中的同名副本；若其他软件使用同名 Mihomo，其连接也可能受影响，安装器会显示覆盖提示。
结束前仍复核活进程的身份，不按 Clash、OpenVPN、WireGuard 等其他名称关闭软件。

无法安全结束进程、恢复自有网络状态，或程序文件与可信清单冲突时，安装会停止并保留现场。
请保留安装日志与恢复材料，按错误提示或[故障排查](../docs/TROUBLESHOOTING.zh-CN.md)处理，
不要删除个人文件来绕过检查。
...
- 客户端普通运行、断开和退出只处理能够确认归属的自有核心；安装/卸载的进程范围见上文。
