...
abstract class LiquidGlassRenderObject extends RenderProxyBox {
  ...
  void paintLiquidGlassWithCapture(
    ...
    // Canvas shaders use local logical coordinates, unlike ImageFilter shaders.
    // Keep background sampling, geometry and optical thickness in those units.
    final thisOriginGlobal = matteTransform.getTranslation();
    final captureOffset = Offset(thisOriginGlobal.x, thisOriginGlobal.y) -
        captureOriginInScreenSpace -
        offset;
    final captureSize = ui.Size(capture.width / dpr, capture.height / dpr);
    final geometryOffsetInCapture = _geometryLocalBounds.topLeft + offset;
    final geometrySizePhysical = _geometryLocalBounds.size;
    const scale = 1.0 / 3.0;
    ...
    // The caller already includes any explicitly requested jelly expansion.
    // Static cards must not paint an unconditional 20x15px fringe.
    final clipRect = boundingBox;
