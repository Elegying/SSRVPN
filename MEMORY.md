...
> 历史项目记录，仅保留 2026-07-04 当时的上下文，其中版本、Release、产物、分支和
> “待发布”结论均已过期。当前事实以 `docs/README.md` 所列文档、当前源码和验证结果为准。
...
- Android 原生 `MainActivity` 已实现启动阶段 `syncSettings` MethodChannel，并把该检查纳入 Android native guard，避免启动时出现误导性的 `MissingPluginException`。
- macOS/Windows 的 `node_edit_screen`、`settings_screen`、`unlock_test_screen`、`subscription_screen` 已抽到 `packages/ssrvpn_shared/lib/desktop_ui/screens/*_part.dart`，平台目录只保留 imports 和 `part` 薄壳。
- 新增共享 `AppLogger`，三端常见运行日志已统一脱敏和 release 降噪；启动日志仍保留专用 `StartupLogger` 文件诊断。
- Lite 崩溃反馈增强：复制崩溃报告时会附带 GitHub Issue 提交入口，仍不依赖 Sentry 或付费服务。
- 新增 `scripts/check-secrets.sh`、`scripts/check-coverage-thresholds.sh`、`scripts/smoke-release-artifacts.sh`、`scripts/performance-baseline.sh`；CI/Release/`make verify` 已接入 secret scan、覆盖率门槛和产物 smoke。
- Android 构建栈已升级到 Gradle `8.14.3`、Android Gradle Plugin `8.11.1`、Kotlin Gradle Plugin `2.2.20`，并把 `jvmTarget` 迁移到新的 Kotlin `compilerOptions` DSL。
- `docs/TESTING.md`、`docs/RELEASE_CHECKLIST.zh-CN.md`、`README.md` 已更新测试、性能基准、发版 smoke、免费发布限制和 SHA256 校验说明。
- 订阅解析、节点编辑、节点写入和配置生成链路已扩展主流协议覆盖：`ss`、`ssr`、`vmess`、`vless`、`trojan`、`anytls`、`hysteria`、`hysteria2/hy2`、`tuic`、`snell`、`socks5`、`http/https`；本地编辑会统一清理空值、转换端口/布尔字段、校验协议必需字段，避免乱码或写出异常 YAML。
- “添加强制代理网站”链路已补测试：完整 URL、通配域名、域名去重、IPv4/IPv6 会规范化为 Mihomo 可识别规则，无效主机名会被过滤。
- 多订阅主页展示已按订阅来源折叠：两个及以上订阅来源时显示订阅名分组，单独导入节点统一归为“单独节点”并置顶不折叠；缓存 YAML 使用 `ssrvpn-subscription` 内部字段记录来源，生成 Mihomo 配置时会自动剔除该字段。
- 新增订阅默认名不再强制使用 `SSRVPN.VIP`；普通订阅默认取 URL host，单节点默认取节点名，同时支持 `vless://`、`trojan://`、`hy2://`、`tuic://` 等非 SSR 单节点直接导入。
- 最近一次本地 `bash scripts/verify-all.sh` 已通过 shared、Android、macOS、Windows 的 analyze、test 和覆盖率门槛。
...
`https://github.com/Elegying/SSRVPN/releases/tag/v2.0.11`
...
- `dist/SSRVPN.dmg` 存在
- `dist/SSRVPN.zip` 存在
- 最新 Release 为 `v2.0.11`，本次准备发布 `v2.0.12`
