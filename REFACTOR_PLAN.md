...
- [x] 提取强制代理站点清洗/校验策略 → `ForceProxySitePolicy`
- [x] 提取日志敏感信息脱敏策略 → `LogRedactor`
...
- [ ] `models/app_settings.dart` — 差异在平台特有设置项，暂缓；公共策略已先下沉到 shared

### Phase 1.5: 运行策略去重 ✅ DONE (2026-06-30)
- [x] 强制代理站点数量、清洗、host 提取、IPv4-only 校验统一到 shared
- [x] 启动日志/Clash 日志敏感字段脱敏统一到 shared
- [x] Android 订阅 HTTP adapter override 接入真实拉取路径，测试注入不再空转
