...
| macOS | `SSRVPN.dmg` | 系统代理；TUN 每次连接需管理员授权 | 免费 ad-hoc 分发，未公证，首次打开会有 Gatekeeper 提示 |
| Windows | `SSRVPN_Setup.exe`、便携版 `SSRVPN.zip` | 系统代理、TUN | 免费未签名分发，可能出现 SmartScreen 未知发布者提示 |
...
常见失败可从原有日志入口打开“诊断与运行日志”：它会显示稳定错误编号、可操作建议和
经过大小限制与脱敏的可复制报告。桌面修复只处理能证明属于 SSRVPN 的代理状态，不会在
连接中强制断开，也不会重置其他代理软件的配置。
...
macOS TUN 的管理员授权只代表本机用户同意本次提权，不能让 macOS 验证发布者身份。本项目固定采用免费 ad-hoc/未公证分发，不购买 Apple 或 Windows 代码签名证书；用户必须从正式来源下载并核对 SHA256。
...
匹配 `v*` 的 tag 会触发 GitHub Actions 构建并上传三端产物及 SHA256。macOS 始终生成 ad-hoc、未公证 DMG，Windows 始终生成未签名安装包和便携包；仓库不保留付费桌面签名自动化。发布前必须保持 `main`、版本号、CHANGELOG 与资产清单一致，并在发布后重新下载校验。
