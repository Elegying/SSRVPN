...
SSRVPN 基于 Mihomo（Clash Meta），提供订阅管理、节点测速、智能分流和实时流量查看。三端采用统一的液态玻璃界面，日常操作集中在“主页”“订阅”和“设置”三页。
...
- **订阅与节点**：支持单独刷新或全部刷新；当前连接节点配置未变时保持连接，刷新失败保留已有节点。可手动选择分组或多个节点测速。
- **智能与全局模式**：按内置及更新后的规则决定直连或代理，国内目标直连，需代理及未知目标经节点；支持自定义强制代理、强制直连网站，安卓另有应用分流名单。
- **连接状态一目了然**：查看当前节点、公网 IPv4、上传与下载速率，以及本次代理累计流量。
- **三端统一界面**：玻璃特效可选无、低、中、高；背景可选流动、纯色（含深蓝、纯黑）或自己添加的图片。
- **设置**：自定义代理端口、检查更新、查看运行日志；设置保存在本机。
- **诊断与更新**：用通俗中文说明连接、订阅和测速失败原因，可展开技术详情或复制脱敏报告；报告包含版本和平台。短暂外部网络检查失败不会直接断开已有连接。

**IPv4 / IPv6 双栈**：两类流量遵循同一套分流规则。代理目标能恢复域名时优先尝试 IPv4，再在有限预算内尝试备用地址。直连 IPv6 需要本地网络支持；走代理的 IPv6-only 网站需要节点支持 IPv6 出口，失败不会自动改为直连。更多说明见[完整功能列表](docs/FEATURES.zh-CN.md)。
