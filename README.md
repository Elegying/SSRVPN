...
| Windows | `SSRVPN_Setup.exe` | 系统代理、TUN | 免费未签名分发，可能出现 SmartScreen 未知发布者提示 |
...
匹配 `v*` 的 tag 会触发 GitHub Actions 构建并上传三端产物及 SHA256。macOS 始终生成 ad-hoc、未公证 DMG，Windows 只生成未签名安装器；仓库不保留付费桌面签名自动化。发布前必须保持 `main`、版本号、CHANGELOG 与资产清单一致，并在发布后重新下载校验。
